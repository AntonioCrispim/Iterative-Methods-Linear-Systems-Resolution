...
int preencheVetor(double *v, int l){                            /* Gera o vetor com chute inicial da solução do sistema com dois componentes, um de baixa e um de alta frequência */
    ...
    for(j=0; j<l; j++){
                          v[j] = 0.5*(sin((16*(j+1)*pi)/(l+1)) + sin((40*(j+1)*pi)/(l+1)));
                          //printf("%lf  \n", v[j]);
                          }                  
    return 0;                         
}

int Jacobi(double **A ,double *f, double *V, int k, int l ){  /* Método de GaussSeidel utilizado para relaxação do sistema*/
      ...
      do{                 
          for(i=0;i<l;i++){                                             
                           ...
                           Aux[i]=(f[i]-Aux[i])/A[i][i];                                 
          }
          
          for(i=0; i<l; i++){
                 V[i] = V[i] + 2.0/3.0*(Aux[i]-V[i]);
                    }
...
int main(void){
  ...
  */
   ...
   plotGraphic("set title 'Bigrid'; set xlabel ''; set ylabel ''; set xrange [0:1]; set yrange [-1:1]; plot 'GrafG1.txt' title 'Sem ajustamento' lt 1 w lp,\ 'GrafG2.txt' title 'Com ajustamento' lt 2 w lp; pause 100");
