...
int preencheMatriz(double **A, int N, int k){                        /* Gera a matriz tridiagonal representante do sistema */
...
int preparacao(double **A, double **I21, double **I12, int k){    
    
    int i, j;
    double **Aux21, **Aux12;
    
  Aux21 = malloc((k-1)/2 * sizeof(double *));              
         if(Aux21 == NULL){
   ...
   for(i=0; i<(k-1)/2; i++){
      Aux21[i] = malloc(((k-1)/2-1)/2 * sizeof(double));
          if(Aux21[i] == NULL){
          printf("Erro de alocacao de memoria\n");
          exit(1);
        }
     ...
     Aux12 = malloc(((k-1)/2-1)/2 * sizeof(double *));              
         if(Aux12 == NULL){
         printf("Erro de alocacao de memoria\n");
         exit(1);
        }

   for(i=0; i<((k-1)/2-1)/2; i++){
      Aux12[i] = malloc((k-1)/2 * sizeof(double));
          if(Aux12[i] == NULL){
          printf("Erro de alocacao de memoria\n");
          exit(1);
        }
   }
   GeraMatrizInterpolacaoRestricao(I21, I12, k);
   GeraMatrizInterpolacaoRestricao(Aux21, Aux12,(k-1)/2);
   
 for(i=0; i<(k-1)/2; i++){                           
           for(j=0;j<((k-1)/2-1)/2;j++){
                                   I21[i+k+1][j] = Aux21[i][j];
     }
 } 
...
int main(void){
     double **A, *f, *V, *r1, *r2, *E1, *E2, **I12, **I21, somaE, Er, max, h;
     int i, j, k, l, cont, N, M;
     FILE  *GrafG1, *GrafG2;
  ...
  */
  ...
  preencheMatriz(A, N, n);
  preparacao(A, I21, I12, n);
 /* preencheVetor(V, n); 
  h=0.0;
  for(i=0; i<n; i++){
